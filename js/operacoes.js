const Operacoes = {
  ...
  renderGastos() {
    ...
    const rows = this._data.gastos.slice(0, 25).map(item => '<tr><td>' + this.escape(Utils.fmtDate(item.data)) + '</td><td><strong>' + this.escape(item.descricao) + '</strong><small>' + this.escape(item.fornecedor || item.nota_fiscal || '') + '</small></td><td><span class="ops-tag">' + this.escape(item.categoria.replace('_', ' ')) + '</span></td><td>' + this.escape(this.quoteLabel(this._data.orcamentos.find(q => q.id === item.orcamento_id))) + '</td><td class="ops-money">' + this.money(item.valor) + '</td><td><button class="ops-icon" data-delete="gastos" data-id="' + Utils.safeId(item.id) + '" title="Excluir">×</button></td></tr>').join('');
  ...
  renderEquipe() {
    ...
    const cards = this._data.equipe.map(person => {
      ...
      return '<article class="ops-person"><div class="ops-person-top"><div class="ops-avatar">' + this.escape(person.nome.charAt(0).toUpperCase()) + '</div><div><strong>' + this.escape(person.nome) + '</strong><span>' + this.escape(person.funcao || 'Freelancer') + '</span></div><button class="ops-icon" data-delete="equipe" data-id="' + Utils.safeId(person.id) + '" title="Excluir">×</button></div><div class="ops-person-meta">' + this.escape(person.telefone || 'Sem telefone') + '<br>Di\u00e1ria padr\u00e3o: <b>' + this.money(person.valor_diaria) + '</b></div><div class="ops-person-foot"><span>' + history.length + ' evento(s)</span><strong>' + this.money(total) + '</strong></div></article>';
    }).join('');
    const rows = this._data.diarias.slice(0, 20).map(item => '<tr><td>' + this.escape(Utils.fmtDate(item.data)) + '</td><td><strong>' + this.escape(item.equipe?.nome) + '</strong><small>' + this.escape(item.funcao_evento || '') + '</small></td><td>' + this.escape(this.quoteLabel(item.orcamentos)) + '</td><td>' + this.escape(item.horario_inicio || '—') + '</td><td class="ops-money">' + this.money(item.valor_diaria) + '</td><td><button class="ops-icon" data-delete="equipe_diarias" data-id="' + Utils.safeId(item.id) + '" title="Excluir">×</button></td></tr>').join('');
  ...
  renderFornecedores() {
    ...
    if (!root) return;
    const cards = this._data.fornecedores.map(item => '<article class="ops-person"><div class="ops-person-top"><div class="ops-avatar ops-avatar-alt">' + this.escape(item.nome.charAt(0).toUpperCase()) + '</div><div><strong>' + this.escape(item.nome) + '</strong><span>' + this.escape(item.tipo) + '</span></div><button class="ops-icon" data-delete="fornecedores" data-id="' + Utils.safeId(item.id) + '" title="Excluir">×</button></div><div class="ops-person-meta">' + this.escape(item.contato || 'Sem contato') + '<br>' + this.escape(item.telefone || 'Sem telefone') + '</div></article>').join('');
    const rows = this._data.eventos.slice(0, 25).map(item => '<tr><td>' + this.escape(Utils.fmtDate(item.data)) + '</td><td><strong>' + this.escape(item.fornecedores?.nome) + '</strong><small>' + this.escape(item.descricao_servico || item.fornecedores?.tipo || '') + '</small></td><td>' + this.escape(this.quoteLabel(item.orcamentos)) + '</td><td>' + this.escape(item.horario_chegada || '—') + '</td><td><span class="ops-tag">' + this.escape(item.status_pagamento) + '</span></td><td class="ops-money">' + this.money(item.valor) + '</td><td><button class="ops-icon" data-delete="fornecedor_eventos" data-id="' + Utils.safeId(item.id) + '" title="Excluir">×</button></td></tr>').join('');
  ...
  modalEquipe() { this.modal('Cadastrar profissional', '<div class="ops-form-grid">' + this.field('Nome completo', 'nome', 'text', true) + this.field('Fun\u00e7\u00e3o principal', 'funcao') + this.field('Telefone / WhatsApp', 'telefone') + this.field('E-mail', 'email', 'email') + this.field('Di\u00e1ria padr\u00e3o (R$)', 'valor_diaria', 'number', true, '0') + this.field('CPF (restrito)', 'cpf') + this.field('RG (restrito)', 'rg') + this.field('Filia\u00e7\u00e3o (restrito)', 'filiacao') + this.text('Observa\u00e7\u00f5es', 'observacoes') + '</div>', async f => Api.request('/rest/v1/equipe', { method: 'POST', body: JSON.stringify(Api.orgPayload({ nome: this.val(f, 'nome'), funcao: this.val(f, 'funcao'), telefone: this.val(f, 'telefone'), email: this.val(f, 'email'), valor_diaria: this.num(f, 'valor_diaria'), cpf: this.val(f, 'cpf', 30), rg: this.val(f, 'rg', 30), filiacao: this.val(f, 'filiacao', 200), observacoes: this.val(f, 'observacoes', 2000) })) })); },
