const Assinatura = {
  ...
  render() {
    ...
    root.querySelectorAll('[data-pix]').forEach(b=>b.addEventListener('click',()=>this.abrirCobrancaPix(b.dataset.pix)));
  ...
  abrirWhats(id) {
    ...
    document.getElementById('billingNewFields').hidden=true; document.getElementById('billingWhatsFields').hidden=false; document.getElementById('billingConfigPixFields').hidden=true; document.getElementById('billingPixPreview').hidden=true;
  ...
  async confirmarModal() {
    ...
    if (!document.getElementById('billingPixPreview').hidden) return this.baixarCobrancaPix();
  ...
  abrirConfigPix() {
    ...
    document.getElementById('billingPixName').value=this.pixConfig?.favorecido||'Alessandro Cesar de Souza Lima';
  ...
  baixarCobrancaPix() {
    ...
    const doc=new jspdf.jsPDF(); doc.setFillColor(25,25,35);doc.rect(0,0,210,48,'F');doc.setDrawColor(217,26,114);doc.setLineWidth(1.5);doc.line(0,48,210,48);doc.setTextColor(255,255,255);doc.setFontSize(22);doc.text('PRYNTIX',18,26);doc.setFontSize(9);doc.text('DOCUMENTO DE COBRANCA',192,18,{align:'right'});doc.setFontSize(18);doc.text('Aviso de cobranca',192,31,{align:'right'});doc.setTextColor(30,30,40);doc.setFontSize(9);doc.text('CLIENTE',18,64);doc.setFontSize(12);doc.text('1000 BEATS AUDIO, VIDEO E ILUMINACAO LTDA.',18,73);doc.setFontSize(9);doc.text('CNPJ 62.496.834/0001-97',18,80);doc.setFillColor(252,240,246);doc.rect(18,90,174,31,'F');doc.setFontSize(9);doc.text('REFERENCIA',24,99);doc.text('VENCIMENTO',87,99);doc.text('VALOR',145,99);doc.setFontSize(12);doc.text(item.competencia,24,111);doc.text(Utils.fmtDate(item.vencimento),87,111);doc.setTextColor(217,26,114);doc.setFontSize(15);doc.text(Utils.fmt(item.valor),145,111);doc.addImage(canvas.toDataURL('image/png'),'PNG',18,134,55,55);doc.setTextColor(30,30,40);doc.setFontSize(14);doc.text('Pagamento via PIX',85,143);doc.setFontSize(10);doc.text('Escaneie o QR Code ou use o PIX Copia e Cola.',85,153);doc.text('Favorecido: '+this.pixConfig.favorecido,85,164);doc.text('Instituicao: '+(this.pixConfig.instituicao||''),85,172);doc.setFontSize(8);doc.setTextColor(110,110,125);doc.text('Este documento e um aviso de cobranca para pagamento via PIX.',18,208);doc.text('O recibo sera disponibilizado apos a confirmacao do pagamento.',18,214);doc.save('cobranca-pix-1kbeats-'+item.competencia.replace('/','-')+'.pdf');
  },
